...
func (s Server) Dist(ctx context.Context, uid *pb.UniqueId) (*pb.QueryResponse, error){


    id, err := strconv.Atoi(uid.Id)
    if err != nil{
	...
	for elem := range merge(c1,c2,c3,c4){
        re := &pb.ResponseElement{
            Id: strconv.Itoa(elem.ID),
            Dist: elem.Dist,
        }
        queryResponse.Response = append(queryResponse.Response, re)
	}
	wg.Wait()

    //Sort by distance
    sort.Sort(ByDistance(queryResponse.Response))
